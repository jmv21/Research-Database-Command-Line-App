...
T.Slider {
    id: control
    property color valueLabelBackgroundColor: Material.accent
    property alias units: valueLabelItem.units
    property alias handleValueLabel: valueLabelItem
    Material.roundedScale: Material.ExtraLargeScale

    implicitWidth: Math.max(implicitBackgroundWidth + leftInset + rightInset,
                            implicitHandleWidth + leftPadding + rightPadding)
    implicitHeight: Math.max(implicitBackgroundHeight + topInset + bottomInset,
                             implicitHandleHeight + topPadding + bottomPadding) + (valueLabelItem.visible ? valueLabelItem.height + padding : 0)
    ...
    Item{
        ...
        height:Math.max(valueLabel.implicitHeight, valueLabelBackground.height)
        width: Math.max(valueLabelBackground.width)
        property int units: -1
        property real valueLabelVisibleWidth: (units > -1) ? valueLabelMetrics.width : -1

        TextMetrics{
            id: valueLabelMetrics
            font: valueLabel.font
            text: "0".repeat(valueLabelItem.units + 2)
        }
        ...
        Rectangle{
            ...
            width: (valueLabelItem.valueLabelVisibleWidth > -1 ? Math.min(valueLabelItem.valueLabelVisibleWidth,valueLabel.implicitWidth) : valueLabel.implicitWidth)
                   + control.padding
            height: valueLabel.implicitHeight + control.padding
            color: control.valueLabelBackgroundColor
            Behavior on width {
        ...
        CustomLabel{
            id: valueLabel
            maximumLineCount: 1
            backgroundColor: control.valueLabelBackgroundColor
            text: control.value
            anchors.centerIn: parent
            horizontalAlignment: Text.AlignHCenter
            width: valueLabelBackground.width
        }
    }
